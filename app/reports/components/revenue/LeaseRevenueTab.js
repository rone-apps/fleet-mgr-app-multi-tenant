...
export default function LeaseRevenueTab({ driverNumber, startDate, endDate }) {
  ...
  // Sorting
  const [sortBy, setSortBy] = useState("date"); // 'date' | 'cab'
  ...
  useEffect(() => {
    if (reportData && reportData.leaseItems) {
      applyFiltersAndSort();
    }
  }, [reportData, searchDriver, searchDate, searchCab, sortBy, sortOrder]);
  ...
  const applyFiltersAndSort = () => {
    let filtered = [...reportData.leaseItems];

    // Apply search filters
    if (searchDriver) {
      filtered = filtered.filter(item =>
        item.driverNumber?.toLowerCase().includes(searchDriver.toLowerCase()) ||
        item.driverName?.toLowerCase().includes(searchDriver.toLowerCase())
      );
    }
  ...
  const handleDownloadCSV = () => {
    ...
    a.href = url;
    a.download = `lease-revenue-${driverNumber}-${format(startDate, "yyyy-MM-dd")}-to-${format(endDate, "yyyy-MM-dd")}.csv`;
    a.click();
  ...
  if (!reportData || !reportData.leaseItems || reportData.leaseItems.length === 0) {
    return (
      <Paper sx={{ p: 6, textAlign: "center" }}>
        ...
        <Typography variant="body2" color="text.secondary">
          No shifts worked by other drivers on your owned shifts in this date range.
        </Typography>
      </Paper>
    );
  }
      ...
      <Paper>
        ...
        >
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Revenue from {reportData.ownerDriverName} ({reportData.ownerDriverNumber})
            </Typography>
          </Box>
          <Button
            variant="outlined"
          ...
          <Table stickyHeader>
            <TableHead>
              <TableRow>
                ...
                <TableCell>Working Driver</TableCell>
